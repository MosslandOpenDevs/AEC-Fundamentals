...
> ⚠️ **Verification status: UNVERIFIED.** The wall/rahmen data mappings below are a simplifying design heuristic, not absolute rules. **Do not use for compliance decisions or model training (RAG).** See [Issue #7](https://github.com/MosslandOpenDevs/AEC-Fundamentals/issues/7).
...
| <strong>[Data] Static Zone</strong><br><br><li>BIM Wall Attribute = <strong>`Structural`</strong> (Fixed)</li><li>Room boundaries are effectively fixed, resulting in low flexibility for the O&M data model.</li> | <strong>[Data] Dynamic Layer</strong><br><br><li>BIM Wall Attribute = <strong>`Architectural`</strong> (Variable)</li><li>Sensor data is commonly mapped to the relatively stable <strong>Columns (Anchors)</strong>, while wall info is more freely modifiable.</li> |
