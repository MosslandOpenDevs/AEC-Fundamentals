...
1. **기획검토 단계**: 건축물의 규모검토, 현장조사, 설계지침 등 선축설계 발주에 필요하여 건축주가 사전에 요구하는 설계업무를 기획하는 단계  
2. **계획설계**: 디자인 개념 및 연관분야의 기본시스템이 검토돈 계획안을 승인받는 단계  
3. **중간설계**: 계획설계 내용을 구체화하여 발전안을 정하고, 실시설계 단계에서의 변경 가능성을 최소화하기 위해 다각적인 검토가 이루어지는 단계. 이 단계에서 건축주로부터 설계도서를 승인받습니다.  
4. **본설계(실시설계)**: 시공을 염두에 둔 설계도서(시방서 포함)들을 작성하는 단계  
5. **시공단계**  
6. **평가단계(POE)**: 완공 이후 사용 후 평가 단계  

---

# 건축선

건축선은 건축할 수 있는 도로 경계선을 일컫는 말입니다.  
...
## 대지면적
대지면적은 건축할 수 있는 땅을 말합니다. 대지면적은 건축물 대장에 표시된 면적만을 가리키며 따라서 이는 토지면적과 같거나 작을 수도 있습니다.
...
건폐율은 지역에 따라 건축법규로 미리 정해져 있으며, 신축 시 전체 면적을 결정 짓는 매우 중요한 요소입니다. 같은 면적의 토지이더라도 건축 면적이 달라지는 이유가 바로 이 건폐율 때문입니다.
...
메소포타미아와 이집트는 세계 5대 고대 건축 문화로 꼽히는 지역들입니다.  
메소포타미아는 현세관에 치중하고, 점성술이 발달하여 제단 겸 신의 주거 개념인 지구라트를 건설했습니다.  
반면 이집트는 영혼 불멸사상과 내세관에 의해 미라와 피라미드를, 신의 주거로서 신전을 건축했습니다.
...
그리스는 고대 오리엔트 문화와 접촉하여 문화의 기틀을 마련했고, 도시국가의 소단위 사회를 바탕으로 각 도시 간의 경쟁적인 정치, 경제 및 문화적 상황 속에서 일찍 고급 문화를 형성할 수 있었습니다.
