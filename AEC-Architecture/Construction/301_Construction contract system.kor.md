...
> ⚠️ **검증 상태: 미검증(unverified).** 이 문서의 도급·하도급 제도 설명(전면 금지 서술 등)은 법정 예외를 포함해 아직 1차 출처로 검증되지 않았습니다. **규정 준수 판단이나 모델 학습(RAG) 근거로 사용하지 마세요.** 관련 논의: [Issue #7](https://github.com/MosslandOpenDevs/AEC-Fundamentals/issues/7).
...
재도급/재하도급/일괄하도급은 건설산업기본법상 **원칙적으로** 금지됩니다(발주자의 서면 승낙 등 일정한 법정 예외가 있으므로, 실제 적용은 조문·시행일 확인이 필요합니다).
