...
> ⚠️ **검증 상태: 미검증(unverified).** 아래 벽식/라멘 데이터 매핑은 단순화된 설계 heuristic이며, 절대 규칙이 아닙니다. **규정 준수 판단이나 모델 학습(RAG) 근거로 사용하지 마세요.** 관련 논의: [Issue #7](https://github.com/MosslandOpenDevs/AEC-Fundamentals/issues/7).
...
| <strong>[데이터] 정적 공간 (Static Zone)</strong><br><br><li>BIM 벽체 속성 = <strong>`Structural`</strong> (고정)</li><li>공간(Room) 경계가 사실상 고정되므로, 운영 단계 데이터 모델의 유연성이 낮습니다.</li> | <strong>[데이터] 동적 레이어 (Dynamic Layer)</strong><br><br><li>BIM 벽체 속성 = <strong>`Architectural`</strong> (가변)</li><li>센서 데이터는 상대적으로 안정적인 <strong>기둥(Anchor)</strong>에 매핑하는 접근이 흔히 쓰이며, 벽체 정보는 비교적 자유롭게 수정 가능합니다.</li> |
