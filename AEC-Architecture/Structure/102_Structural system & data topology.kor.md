...
| <strong>[데이터] 정적 공간 (Static Zone)</strong><br><br><li>BIM 벽체 속성 = <strong>`Structural`</strong> (고정)</li><li>공간(Room) 경계가 영구 고정되므로, 운영 단계 데이터 모델의 유연성이 낮습니다.</li> | <strong>[데이터] 동적 레이어 (Dynamic Layer)</strong><br><br><li>BIM 벽체 속성 = <strong>`Architectural`</strong> (가변)</li><li>센서 데이터는 변하지 않는 <strong>기둥(Anchor)</strong>에 매핑되며, 벽체 정보는 언제든 수정 가능합니다.</li> |
